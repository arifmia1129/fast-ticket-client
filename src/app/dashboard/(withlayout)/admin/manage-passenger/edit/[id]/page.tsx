...
const EditPassengerPage = ({ params }: any) => {
  const [updatePassenger] = useUpdatePassengerMutation();

  const { data: passengerData } = useGetPassengerByIdQuery(params.id);
  ...
  const onSubmit = async (info: any) => {
    ...
    try {
      ...
      if (data?._id) {
        message.success("Successfully updated passenger account");
      } else {
  ...
  return (
    <div>
      <Form submitHandler={onSubmit} defaultValues={passengerData}>
        <div
          style={{
